...
object Main  {
    def lineCheck(line: String): Boolean = {
        ...
        else if (!line(line.length-1).isDigit) false
    ...
    def getTopCount(x: String, y: String): String = {
        val xNum = x(x.length - 1).toInt
        val yNum = y(y.length - 1).toInt
    ...
    def formatForFile (line: String): String = {
        ...
        val result = line(0) + "," + line(line.length - 1) + "," + items
        result
    }
    ...
    def main(args: Array[String]): Unit = {
        ...
        val input = sc.textFile("../twitter-stream/Results/part-00000")
          .filter(lineCheck)
        ...
        val file = saveAsFile(fileResults, s"FinalResults/HashtagResults-$now.csv")
        val s3Path = "cpiazza01-revature/project2/finalStreamingResults"
        val key = sys.env.get("AWS_KEY").get
        val secret = sys.env.get("AWS_SECRET").get
